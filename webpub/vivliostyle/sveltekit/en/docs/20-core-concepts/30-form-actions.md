...
Redirects (and errors) work exactly the same as in [`load`](../20-core-concepts/20-load.html#redirects):
...
Form actions are the preferred way to send data to the server, since they can be progressively enhanced, but you can also use [`+server.js`](../20-core-concepts/10-routing.html#server) files to expose (for example) a JSON API.
