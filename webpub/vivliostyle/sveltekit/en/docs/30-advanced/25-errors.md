...
An _expected_ error is one created with the [`error`](../50-api-reference/30-modules.html#sveltejs-kit-error) helper imported from `@sveltejs/kit`:
...
This tells SvelteKit to set the response status code to 404 and render an [`+error.svelte`](../20-core-concepts/10-routing.html#error) component, where `$page.error` is the object provided as the second argument to `error(...)`.
...
Unexpected errors will go through the [`handleError`](../30-advanced/20-hooks.html#shared-hooks-handleerror) hook, where you can add your own error handling — for example, sending errors to a reporting service, or returning a custom error object. 
...
If an error occurs inside `handle` or inside a [`+server.js`](../20-core-concepts/10-routing.html#server) request handler, SvelteKit will respond with either a fallback error page or a JSON representation of the error object, depending on the request's `Accept` headers.
...
If the error instead occurs inside a `load` function while rendering a page, SvelteKit will render the [`+error.svelte`](../20-core-concepts/10-routing.html#error) component nearest to where the error occurred.
...
This interface always includes a `message: string` property.<span style='float: footnote;'><a href="../../index.html#toc">Go to TOC</a></span>
