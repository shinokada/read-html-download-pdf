...
When you're creating an app, the contents of `src/routes` is the public-facing stuff; [`src/lib`](../50-api-reference/30-modules.html#$lib) contains your app's internal library.
...
Running the `svelte-package` command from `@sveltejs/package` will take the contents of `src/lib` and generate a `package` directory (which can be [configured](../50-api-reference/10-configuration.html#package)) containing the following:

- All the files in `src/lib`, unless you [configure](../50-api-reference/10-configuration.html#package) custom `include`/`exclude` options. Svelte components will be preprocessed, TypeScript files will be transpiled to JavaScript.
- Type definitions (`d.ts` files) which are generated for Svelte, JavaScript and TypeScript files. You need to install `typescript >= 4.0.0` for this. Type definitions are placed next to their implementation, hand-written `d.ts` files are copied over as is. You can [disable generation](../50-api-reference/10-configuration.html#package), but we strongly recommend against it.
...
The `"exports"` field contains the package's entry points. By default, all files in `src/lib` will be treated as an entry point unless they start with (or live in a directory that starts with) an underscore, but you can [configure](../50-api-reference/10-configuration.html#package) this behaviour. If you have a `src/lib/index.js` or `src/lib/index.svelte` file, it will be treated as the package root.
...
The `./package` above is referring to the directory name generated, change accordingly if you configure a custom [`package.dir`](../50-api-reference/10-configuration.html#package).
