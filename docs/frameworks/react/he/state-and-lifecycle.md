...
דף זה מציג את הקונספט של state ומחזור חיים בקומפוננטת React. תוכלו למצוא את [פירוט ה-API של קומפוננטה כאן](./react-component.html).

הביטו על דוגמת השעון המתקתק מ[אחד מהחלקים הקודמים](./rendering-elements.html#updating-the-rendered-element). ב[רינדור אלמנטים](./rendering-elements.html#rendering-an-element-into-the-dom), למדנו רק דרך אחת לעדכון ממשק המשתמש. אנו קוראים ל-`ReactDOM.render()` כדי לשנות את הפלט שירונדר:
