...
מדריך עזר זה מסביר על מעטפת ה-`SyntheticEvent` המהווה חלק ממערכת האירועים של React. ראה את מדריך [טיפול באירועים](./handling-events.html) על מנת ללמוד עוד.
...
> הערה:
>
> מגרסה v17, שימוש ב- `e.persist()` לא עושה כלום בגלל שה- `SyntheticEvent` לא [מצורף יותר](./legacy-event-pooling.html).
...
לקבלת מידע נוסף על אירוע ה-onChange, ראה [טפסים](./forms.html).
