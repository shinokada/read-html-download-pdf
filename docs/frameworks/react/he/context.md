...
**אם המטרה היחידה שלך בשימוש בקונטקסט היא להמנע מהעברת props להרבה קומפוננטות, [הכלת קומפוננטות](./composition-vs-inheritance.html) היא בדרך כלל פתרון פשוט יותר.**
...
דרך אחת לפתור את הבעיה **ללא שימוש בקונטקסט** היא [להעביר את קומפוננטת ה-`Avatar` עצמה](./composition-vs-inheritance.html#containment) כדי שקומפוננטות הביניים לא יצטרכו לדעת על ה-props `user` או `avatarSize`:
...
אין הגבלה של ילד יחיד לכל קומפוננטה. אפשר להעביר ילדים מרובים, ואפילו ״משבצות״ ("slots") מרובות לילדים, [כמתועד כאן](./composition-vs-inheritance.html#containment):
...
תבנית העיצוב הזאת מספיקה במקרים רבים כשרוצים להפריד קומפוננטת ילד מקומפוננטת האב שלה. אפשר להרחיב את הפתרון עוד יותר עם [render props](./render-props.html) במקרים שקומפוננטת הילד צריכה לתקשר עם קומפוננטת האב לפני הרינדור.
...
כל הצרכנים שהם צאצאים של ספק, ירונדרו בכל שינוי ב-prop `value`. ההפצה מהספק לצאצאיו הצרכנים (כולל [.contextType](./context.html#classcontexttype) ו-[useContext](./hooks-reference.html#usecontext)) אינה כפופה למתודה `shouldComponentUpdate`, כך שהצרכן מתעדכן אפילו כשקומפוננטה מעליו מדלגת על העדכון.  
...
מאפיין ה-`contextType` במחלקה מוקצה בעצם קונטקסט שנוצר על ידי המתודה [`React.createContext()`](#reactcreatecontext). שימוש במאפיין זה נותן לנו לצרוך את ערך הקונטקסט הנוכחי הקרוב ביותר בעזרת `this.context`. אפשר להשתמש בהפניה זו בכל אחת ממתודות מחזור החיים כולל מתודת הרינדור.
...
קומפוננטת React שמקשיבה לשינויים בקונטקסט. שימוש בקומפוננטה זו מאפשר לצרוך קונטקסט מתוך [קומפוננטת פונקציה](./components-and-props.html#function-and-class-components).

דורשת [פונקציה בתור ילד](./render-props.html#using-props-other-than-render). הפונקציה הזאת מקבלת את ערך הקונטקסט הנוכחי ומחזירה צומת React. ערך הארגומנט שמועבר לפונקציה יהיה זהה ל- `value` prop של ספק הקונטקסט הקרוב ביותר מעלינו בעץ. אם אין ספק לקונטקס, הערך יהיה זהה לערך ברירת המחדל שנקבע בזמן יצירת הקונטקסט (עם `createContext()`).

> הערה
> 
> למידע נוסף על תבנית ״פונקציה כילד״ בקרו בעמוד [render props](./render-props.html).
...
> הערה
> 
> בעבר, React הכילה ממשק תכנות נסיוני לקונטקסט. הממשק הישן ייתמך בכל גרסאות ה-16.x, אבל אפליקציות שמשתמשות בו צריכות לעבור לשימוש בגרסה החדשה. הממשק הישן יוסר בגרסה הראשית הבאה של React. עוד מידע על [ממשק הקונטקסט הישן](./legacy-context.html).
