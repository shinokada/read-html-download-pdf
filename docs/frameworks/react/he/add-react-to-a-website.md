...
>קוד זה מגדיר קומפוננטת React שנקראת `LikeButton`. אל תדאג אם אתה לא מבין את זה עדיין -- אנו נכסה את היסודות של React מאוחר יותר ב[מדריך המעשי](/tutorial/tutorial.html) וב[מדריך הקונספטים העיקריים](./hello-world.html). בינתיים, בוא רק נדאג שזה יוצג על המסך!
...
>הערה
>
>אסטרטגיה זו היא שימושית בעיקר כשחלקים בעמוד שמשתמשים ב-React מבודדים אחד מן השני. בתוך קוד React, קל יותר להשתמש [בחיבור קומפוננטות](./components-and-props.html#composing-components) במקום.
...
לעומת זאת, ריאקט מציע אפשרות להשתמש ב-[JSX](./introducing-jsx.html) במקום:
...
שתי דוגמאות הקוד זהות. למרות **ש-JSX  [הוא אופציונאלי לגמרי](./react-without-jsx.html)**, מספר רב של אנשים מוצאים אותו יעיל עבור כתיבת קוד ממשק משתמש -- ב-React וגם בספריות אחרות.
...
אם אתה שם לב שאתה מתחיל להרגיש בנוח יותר עם כלי בנייה ואתה רוצה שהם יעשו יותר עבורך, [החלק הבא](./create-a-new-react-app.html) מסביר על הכלים הפופולריים והנגישים ביותר. אם לא -- תגיות הסקריפט הללו יעשו את העבודה מצויין!
