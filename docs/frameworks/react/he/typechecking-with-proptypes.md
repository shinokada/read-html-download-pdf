...
In this example, we are using a class component, but the same functionality could also be applied to function components, or components created by [`React.memo`](./react-api.html#reactmemo) or [`React.forwardRef`](./react-api.html#reactforwardref).
