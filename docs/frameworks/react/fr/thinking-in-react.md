...
Il existe deux types de données dans le « modèle » de React : les props et l'état local. Il est important de bien comprendre la distinction entre les deux ; jetez un coup d’œil à [la doc officielle de React](./state-and-lifecycle.html) si vous n'êtes pas sûr·e de la différence. Vous pouvez aussi consulter la [FAQ : Quelle est la différence entre `state` et `props` ?](./faq-state.html#what-is-the-difference-between-state-and-props)
