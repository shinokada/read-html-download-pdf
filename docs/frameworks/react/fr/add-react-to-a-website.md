...
>Remarque
>
>Cette solution est souvent utilisée lorsque les éléments de React doivent être isolés les uns des autres. Il est cependant plus facile d'utiliser le principe de [composition de composants](./components-and-props.html#composing-components) au sein de votre code React.
...
Néanmoins, React nous offre aussi la possibilité d'utiliser plutôt [JSX](./introducing-jsx.html).
...
Ces deux extraits de code sont équivalents. Même si la syntaxe **JSX est [complètement optionnelle](./react-without-jsx.html)**, la plupart des développeur·euse·s React la trouvent très pratique pour écrire du code  (avec React ou même avec d'autres bibliothèques).
