...
Avec Suspense, gérer les erreurs de chargement fonctionne comme la gestion des erreurs de rendu : vous pouvez utiliser un [périmètre d’erreur](./error-boundaries.html) où bon vous semble pour « attraper » les erreurs dans les composants qu’il enrobe.
...
Pour répondre à ces questions, nous vous invitons à lire la prochaine section sur les [Approches pour une UI concurrente](./concurrent-mode-patterns.html).
