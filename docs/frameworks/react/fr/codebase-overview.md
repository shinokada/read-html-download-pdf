...
Si vous souhaitez [contribuer à React](./how-to-contribute.html), nous espérons que ce guide vous aidera à vous sentir plus à l'aise pour apporter des modifications.
...
* `build` est la sortie de construction de React. Il ne figure pas dans le dépôt, mais il apparaîtra dans votre clone de React après que vous [l'aurez construit](./how-to-contribute.html#development-workflow) pour la première fois.
...
Le « noyau » de React inclut toutes les [API `React` de niveau racine](./top-level-api.html#react), par exemple :

* `React.createElement()`
* `React.Component`
* `React.Children`

**Le noyau React n'inclut que les API nécessaires à la définition des composants.**  Il n'inclut pas l'algorithme de [réconciliation](./reconciliation.html) ni aucun code spécifique à une plate-forme. Il est utilisé à la fois par les composants de React DOM et de React Native.
...
Le réconciliateur _“stack”_ est l'implémentation qui sous-tend React 15 et les versions antérieures. Nous avons depuis cessé de l'utiliser, mais il reste décrit en détail dans la [prochaine page](./implementation-notes.html).
...
Lisez la [prochaine page](./implementation-notes.html) pour en apprendre davantage sur l’implémentation du réconciliateur utilisé avant React 16. Nous n'avons pas encore documenté les détails internes d’implémentation du nouveau réconciliateur.
