...
Dans ces cas-là, nous devrions plutôt utiliser les [fragments React](./fragments.html) pour regrouper de multiples éléments.
...
Lorsque vous n'avez pas besoin de définir de propriété sur la balise fragment, vous pouvez utiliser la [syntaxe courte](./fragments.html#short-syntax), si votre outillage la prend en charge :
...
Pour plus d'informations, consultez [la documentation sur les fragments](./fragments.html).
...
<img src="./docs/keyboard-focus.png" alt="Le contour bleu du focus clavier autour d'un lien sélectionné" />
...
Afin de définir le focus en React, nous pouvons utiliser les [Refs aux éléments du DOM](./refs-and-the-dom.html).
...
Il arrive parfois qu'un composant parent ait besoin de définir le focus sur un élément au sein d’un composant enfant. Pour ce faire, nous [exposons des refs DOM aux composants parents](./refs-and-the-dom.html#exposing-dom-refs-to-parent-components) via une propriété spéciale sur le composant enfant qui transfère la ref du parent au nœud DOM de l'enfant.
...
Quand vous enrobez des composants à l’aide d’un composant d’ordre supérieur *(Higher-Order Component, ou HOC, NdT)*, il est recommandé de [transférer la référence](./forwarding-refs.html) vers l’élément enrobé grâce à la fonction `forwardRef` de React. Si un HOC tiers n'implémente pas le transfert de référence, le modèle ci-dessus peut être utilisé comme solution de secours.
...
<img src="./docs/outerclick-with-mouse.gif" alt="Un bouton ouvrant une liste déroulante implémenté par le modèle du clic externe et déclenché par la souris montrant que l'action de fermeture fonctionne." />
...
<img src="./docs/outerclick-with-keyboard.gif" alt="Un bouton ouvrant une liste déroulante implémenté par le modèle du clic externe et déclenché par le clavier montrant que le menu déroulant ne se ferme pas à la perte de focus et qu'il masque d'autres éléments de l'écran." />
...
<img src="./docs/blur-popover-close.gif" alt="Une liste déroulante se fermant correctement pour les utilisateurs de souris et de clavier." />
