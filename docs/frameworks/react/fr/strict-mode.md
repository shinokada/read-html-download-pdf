...
[Vous pouvez en apprendre davantage sur l'API `createRef` ici.](./refs-and-the-dom.html)

### Signaler l'utilisation dépréciée de `findDOMNode` {#warning-about-deprecated-finddomnode-usage}

React proposait autrefois `findDOMNode` pour rechercher dans l'arborescence le nœud DOM associé à une instance de classe. Normalement, vous n'avez pas besoin de ça car vous pouvez [attacher directement une ref à un nœud du DOM](./refs-and-the-dom.html#creating-refs).
...
Préférez une approche explicite en passant une ref à votre composant personnalisé et en la transférant au DOM grâce au [transfert de ref](./forwarding-refs.html#forwarding-refs-to-dom-components).
...
Lisez la [documentation de la nouvelle API de Contexte](./context.html) pour faciliter la migration vers cette nouvelle version.
