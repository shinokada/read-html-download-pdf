...
React prend en charge tous les navigateurs populaires, y compris Internet Explorer 9 et au-delà, même si [certains polyfills sont nécessaires](./javascript-environment-requirements.html) pour les navigateurs plus anciens tels qu’IE 9 et IE 10.
...
Affiche un élément React au sein du nœud DOM spécifié par `container` et renvoie une [référence](./more-about-refs.html) sur le composant (ou renvoie `null` pour les [fonctions composants](./components-and-props.html#function-and-class-components)).
...
Similaire à [`render()`](#render), mais sert à hydrater un conteneur dont le HTML a déjà été produit par [`ReactDOMServer`](./react-dom-server.html). React tentera alors d’associer les gestionnaires d’événements au balisage existant.
...
Crée un portail.  Les portails fournissent un moyen d’[afficher des enfants dans un nœud du DOM à l’extérieur de la hiérarchie DOM du composant](./portals.html).
