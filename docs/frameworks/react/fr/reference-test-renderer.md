...
De la même manière que l'[utilitaire `act()` de `react-dom/test-utils`](./test-utils.html#act), `TestRenderer.act` prépare un composant permettant la vérification d'attentes. Utilisez cette version de `act()` pour englober les appels à `TestRenderer.create` et `testRenderer.update`.
