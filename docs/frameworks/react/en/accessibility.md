...
In these cases we should rather use [React Fragments](./fragments.html) to group together multiple elements.
...
When you don't need any props on the Fragment tag you can use the [short syntax](./fragments.html#short-syntax), if your tooling supports it:
...
For more info, see [the Fragments documentation](./fragments.html).
...
Sometimes a parent component needs to set focus to an element in a child component. We can do this by [exposing DOM refs to parent components](./refs-and-the-dom.html#exposing-dom-refs-to-parent-components) through a special prop on the child component that forwards the parent's ref to the child's DOM node.
...
When using a [HOC](./higher-order-components.html) to extend components, it is recommended to [forward the ref](./forwarding-refs.html) to the wrapped component using the `forwardRef` function of React. If a third party HOC does not implement ref forwarding, the above pattern can still be used as a fallback.
