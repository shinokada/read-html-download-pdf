...
- The [`recipes`](./testing-recipes.html) document contains more details on how `act()` behaves, with examples and usage.
...
`Simulate` has a method for [every event that React understands](./events.html#supported-events).
