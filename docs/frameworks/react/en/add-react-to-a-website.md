...
>Tip
>
>This code defines a React component called `LikeButton`. Don't worry if you don't understand it yet -- we'll cover the building blocks of React later in our [hands-on tutorial](/tutorial/tutorial.html) and [main concepts guide](./hello-world.html). For now, let's just get it showing on the screen!
...
>Note
>
>This strategy is mostly useful while React-powered parts of the page are isolated from each other. Inside React code, it's easier to use [component composition](./components-and-props.html#composing-components) instead.
...
However, React also offers an option to use [JSX](./introducing-jsx.html) instead:
...
These two code snippets are equivalent. While **JSX is [completely optional](./react-without-jsx.html)**, many people find it helpful for writing UI code -- both with React and with other libraries.
...
If you notice that you're getting comfortable with build tools and want them to do more for you, [the next section](./create-a-new-react-app.html) describes some of the most popular and approachable toolchains. If not -- those script tags will do just fine!
