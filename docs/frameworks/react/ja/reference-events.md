...
このリファレンスガイドでは、React のイベントシステムの一部を構成する `SyntheticEvent`（合成イベント）ラッパについて説明します。詳細については、[イベント処理](./handling-events.html)ガイドを参照してください。
...
> 補足
>
> v17 以降、`SyntheticEvent` は[プーリング](./legacy-event-pooling.html)されなくなったため、`e.persist()` は何も行わなくなります。
...
onChange イベントの詳細については、[Forms](./forms.html) を参照してください。
