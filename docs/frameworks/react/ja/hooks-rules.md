...
- ✅ カスタムフック内（[次のページで説明します](./hooks-custom.html)）から呼び出す。
...
このプラグインは [Create React App](./create-a-new-react-app.html#create-react-app) ではデフォルトで含まれています。
...
**次のページまで飛ばして[独自のフック](./hooks-custom.html)を書く方法について学んでも構いません。**このページの続きの部分ではこれらのルールの背後にある根拠について述べていきます。

## 解説 {#explanation}

[既に学んだ通り](./hooks-state.html#tip-using-multiple-state-variables)、ひとつのコンポーネント内で複数の state や副作用を使うことができます。
...
ついに[自分独自のフックの書き方](./hooks-custom.html)について学ぶ準備ができました！ カスタムフックを使えば React から提供されるフックを組み合わせて自分独自の抽象化を作り出し、複数の異なるコンポーネント間で state を使う共通のロジックを再利用することができるようになります。
