...
本機能の目的は、アプリケーション中の、低速で[メモ化などの最適化](./hooks-faq.html#how-to-memoize-calculations)が有効な可能性のある部位を特定する手助けをすることです。

> 補足:
>
> プロファイリングには追加のオーバーヘッドが生じますので、**[本番ビルド](./optimizing-performance.html#use-the-production-build)では無効化されます**。
>
...
これが（[`React.memo`](./react-api.html#reactmemo)、[`useMemo`](./hooks-reference.html#usememo)、[`shouldComponentUpdate`](./hooks-faq.html#how-do-i-implement-shouldcomponentupdate) などの）メモ化をどれだけうまく有効に使えているかの指標となります。
