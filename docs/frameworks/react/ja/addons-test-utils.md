...
- [`recipes`](./testing-recipes.html) ドキュメントには `act()` がどのように動作するのかについて例や使用法を交えた詳しい解説があります。
...
`Simulate` は [React が理解している全てのイベント](./events.html#supported-events)それぞれに対応するメソッドを持っています。
