...
このステップを実施する上で助けが必要な場合は、[React ドキュメント](./getting-started.html)を参照してください。

### 幕間：Props vs State {#a-brief-interlude-props-vs-state}

React には 2 種類の「モデル」データが存在します。props と state です。このふたつの相違を理解するのは重要なことです。違いについて自信がない場合は、[公式の React ドキュメント](./state-and-lifecycle.html)に目を通すとよいでしょう。[FAQ: state と props の違いは何ですか？](./faq-state.html#what-is-the-difference-between-state-and-props) もご覧ください。
