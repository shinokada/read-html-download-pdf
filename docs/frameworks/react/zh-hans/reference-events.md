...
此参考指南记录了构成 React 事件系统一部分的 `SyntheticEvent` 包装器。请参考有关[事件处理](./handling-events.html)的指南来了解更多。
...
> 注意：
>
> 从 v17 开始，`e.persist()` 将不再生效，因为 `SyntheticEvent` 不再放入[事件池](./legacy-event-pooling.html)中。
...
想了解 onChange 事件的更多信息，查看 [Forms](./forms.html) 。
