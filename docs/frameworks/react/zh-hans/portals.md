...
第一个参数（`child`）是任何[可渲染的 React 子元素](./react-component.html#render)，例如一个元素，字符串或 fragment。第二个参数（`container`）是一个 DOM 元素。
...
> 注意:
> 
> 当在使用 portal 时, 记住[管理键盘焦点](./accessibility.html#programmatically-managing-focus)就变得尤为重要。
>
