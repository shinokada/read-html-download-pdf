...
我们先来回顾一下前一篇关于 [Suspense 用于数据获取](./concurrent-mode-suspense.html) 文章中的 [这个示例](https://codesandbox.io/s/infallible-feather-xjtbu)。
...
首先，要确保项目中正在使用 Concurrent 模式。我们会在稍后讨论如何 [采用 Concurrent 模式](./concurrent-mode-adoption.html)，但是就现在而言，我们要让这个特性工作只要知道需要使用 `ReactDOM.createRoot()` 而非 `ReactDOM.render()` 就足够了：
...
这原因就在于 Concurrent 模式本身。我们 [之前提到](./concurrent-mode-intro.html#intentional-loading-sequences) 它有点像在 “branch” 上运行的的一个 state 更新。或者我们可以想象成，当我们把 state 更新包裹在 `startTransition` 的时候会在*“另一个宇宙中”*开始渲染，就像科幻电影一样。我们并不能直接看到那个宇宙 -- 但是我们能够从那个宇宙探知一些事情正在发生的事情（`isPending`）。当更新完成的时候，我们的“多个宇宙”合并成一个，我们在屏幕上看到最终的结果！
...
正如我们从 [Suspense 简介](./concurrent-mode-suspense.html) 中所了解，所有所需数据没有准备好的组件都可以 “suspend” 一段时间。我们可以从策略上用 `<Suspense>` 把树的不同部分圈起来处理，但这并不总是足够的。
...
<img src="./docs/cm-steps-simple.png" alt="Three steps" />
...
这是通过多年的调查和开发的结果，但它尚未完结。在 [采用 Concurrent 模式](./concurrent-mode-adoption.html) 中，我们会讲如何使用它以及它的效果。
