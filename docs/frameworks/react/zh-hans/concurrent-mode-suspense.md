...
在 Suspense 中，获取数据时抛出的错误和组件渲染时的报错处理方式一样——你可以在需要的层级渲染一个[错误边界](./error-boundaries.html)组件来“捕捉”层级下面的所有的报错信息。
...
对于上述问题的解答，我们将交由下一章节 [Concurrent UI 模式](./concurrent-mode-patterns.html)处理。
