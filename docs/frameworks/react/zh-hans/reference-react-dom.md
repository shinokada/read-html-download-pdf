...
> 注意：
>
> 在 React 18，`render` 和 `hydrate` 都被新的 [客户端方法](./react-dom-client.html) 所取代。这些方法将警告你的应用程序将表现得像运行 React 17 一样（欲了解更多请参阅 [此处](https://reactjs.org/link/switch-to-createroot)）。

### 浏览器支持 {#browser-support}

React 支持所有的现代浏览器，尽管对于旧版本来说，可能需要引入 [相关的 polyfills 依赖](./javascript-environment-requirements.html)。
...
创建 portal。[Portal](./portals.html) 提供了一种将子节点渲染到已 DOM 节点中的方式，该节点存在于 DOM 组件的层次结构之外。
...
> 注意：
>
> 在 React 18 中，`render` 函数已被 `createRoot` 函数所取代。具体请参阅 [createRoot](./react-dom-client.html#createroot) 以了解更多。

在提供的 `container` 里渲染一个 React 元素，并返回对该组件的[引用](./more-about-refs.html)（或者针对[无状态组件](./components-and-props.html#function-and-class-components)返回 `null`）。
...
> 如果你真的需要获得对根组件 `ReactComponent` 实例的引用，那么推荐为根元素添加 [callback ref](./refs-and-the-dom.html#callback-refs)。
>
> 使用 `render()` 对服务端渲染容器进行 hydrate 操作的方式已经被废弃，并且会在 React 17 被移除。作为替代，请使用 [`hydrateRoot()`](./react-dom-client.html#hydrateroot)。
...
> 注意：
>
> 在 React 18 中，请使用 `hydrateRoot` 来替代 `hydrate`。请参阅 [hydrateRoot](./react-dom-client.html#hydrateroot) 以了解更多。

与 [`render()`](#render) 相同，但它用于在 [`ReactDOMServer`](./react-dom-server.html) 渲染的容器中对 HTML 的内容进行 hydrate 操作。React 会尝试在已有标记上绑定事件监听器。
...
> 注意：
>
> 在 React 18 中，`unmountComponentAtNode` 已被 `root.unmount()` 取代。具体请参阅 [createRoot](./react-dom-client.html#createroot) 以了解更多。
...
> 注意:
>
> `findDOMNode` 是一个访问底层 DOM 节点的应急方案（escape hatch）。在大多数情况下，不推荐使用该方法，因为它会破坏组件的抽象结构。[严格模式下该方法已弃用。](./strict-mode.html#warning-about-deprecated-finddomnode-usage)
