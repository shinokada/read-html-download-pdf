...
以前，React 提供了两种方法管理 refs 的方式：已过时的字符串 ref API 的形式及回调函数 API 的形式。尽管字符串 ref API 在两者中使用更方便，但是它有[一些缺点](https://github.com/facebook/react/issues/1373)，因此官方推荐采用[回调的方式](./refs-and-the-dom.html#legacy-api-string-refs)。
...
[在此处了解有关 `createRef` API 的更多信息](./refs-and-the-dom.html)

### 关于使用废弃的 findDOMNode 方法的警告 {#warning-about-deprecated-finddomnode-usage}

React 支持用 `findDOMNode` 来在给定 class 实例的情况下在树中搜索 DOM 节点。通常你不需要这样做，因为你可以[将 ref 直接绑定到 DOM 节点](./refs-and-the-dom.html#creating-refs)。
...
你可以通过将 ref 传递给自定义组件并使用 [ref 转发](./forwarding-refs.html#forwarding-refs-to-dom-components)来将其传递给 DOM 节点。
...
阅读[新的 context API 文档](./context.html)以帮助你迁移到新版本。
