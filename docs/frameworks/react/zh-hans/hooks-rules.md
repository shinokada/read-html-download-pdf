...
* ✅ 在自定义 Hook 中调用其他 Hook (我们将会在[下一页](./hooks-custom.html) 中学习这个。)
...
我们打算后续版本默认添加此插件到 [Create React App](./create-a-new-react-app.html#create-react-app) 及其他类似的工具包中。
...
**现在你可以跳转到下一章节学习如何编写[你自己的 Hook](./hooks-custom.html)。**在本章节中，我们将继续解释这些规则背后的原因。

## 说明 {#explanation}

正如我们[之前学到](./hooks-state.html#tip-using-multiple-state-variables)的，我们可以在单个组件中使用多个 State Hook 或 Effect Hook：
...
最后，接下来会学习[如何编写自定义 Hook](./hooks-custom.html)！自定义 Hook 可以将 React 中提供的 Hook 组合到定制的 Hook 中，以复用不同组件之间常见的状态逻辑。
