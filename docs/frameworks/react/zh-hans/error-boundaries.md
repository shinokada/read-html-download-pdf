...
如果一个 class 组件中定义了 [`static getDerivedStateFromError()`](./react-component.html#static-getderivedstatefromerror) 或 [`componentDidCatch()`](./react-component.html#componentdidcatch) 这两个生命周期方法中的任意一个（或两个）时，那么它就变成一个错误边界。当抛出错误后，请使用 `static getDerivedStateFromError()` 渲染备用 UI ，使用 `componentDidCatch()` 打印错误信息。
