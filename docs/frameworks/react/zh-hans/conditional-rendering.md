...
在下面的示例中，我们将创建一个名叫 `LoginControl` 的[有状态的组件](./state-and-lifecycle.html#adding-local-state-to-a-class)。
...
通过花括号包裹代码，你可以[在 JSX 中嵌入表达式](./introducing-jsx.html#embedding-expressions-in-jsx)。这也包括 JavaScript 中的逻辑与 (&&) 运算符。它可以很方便地进行元素的条件渲染：
...
就像在 JavaScript 中一样，你可以根据团队的习惯来选择可读性更高的代码风格。需要注意的是，如果条件变得过于复杂，那你应该考虑如何[提取组件](./components-and-props.html#extracting-components)。
