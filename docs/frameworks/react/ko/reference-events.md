...
이 문서는 React의 이벤트 시스템 일부를 구성하는 `SyntheticEvent` 래퍼를 설명합니다. 더 많은 정보는 [이벤트 처리하기](./handling-events.html) 문서를 보세요.
...
> 주의
>
> v17부터 `e.persist()`는 `SyntheticEvent`가 더 이상 [풀링](./legacy-event-pooling.html)되지 않기 때문에 아무런 동작을 하지 않습니다.
...
onChange 이벤트에 대한 더 자세한 정보는 [폼 문서](./forms.html)를 참조하세요.
