...
- [테스트 방법](./testing-recipes.html) 문서에 `act()`의 동작 방식에 대한 자세한 내용이 예시와 사용법과 함께 포함되어 있습니다.
...
`Simulate`는 [React가 이해하는 모든 이벤트](./events.html#supported-events)를 위한 메서드를 가집니다.
