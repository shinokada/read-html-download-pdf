...
아래의 예시에서는 `LoginControl`이라는 [유상태 컴포넌트](./state-and-lifecycle.html#adding-local-state-to-a-class) 를 만들 것입니다.
...
JSX 안에는 중괄호를 이용해서 [표현식을 포함](./introducing-jsx.html#embedding-expressions-in-jsx) 할 수 있습니다. 그 안에 JavaScript의 논리 연산자 `&&`를 사용하면 쉽게 엘리먼트를 조건부로 넣을 수 있습니다.
...
JavaScript와 마찬가지로, 가독성이 좋다고 생각하는 방식을 선택하면 됩니다. 또한 조건이 너무 복잡하다면 [컴포넌트를 분리](./components-and-props.html#extracting-components)하기 좋을 때 일 수도 있다는 것을 기억하세요.
