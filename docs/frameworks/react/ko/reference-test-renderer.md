...
[`react-dom/test-utils`의 `act()`](./test-utils.html#act)와 비슷하게, `TestRenderer.act`는 검증을 위한 컴포넌트들을 준비합니다. `TestRenderer.create`와 `TestRenderer.update`의 호출을 이 버전의 `act()`를 사용해서 감싸주세요.
