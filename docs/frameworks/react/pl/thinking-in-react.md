...
Jeśli potrzebujesz pomocy na tym etapie budowy aplikacji, zajrzyj do [Dokumentacji Reacta](./getting-started.html).

### Krótki przerywnik: atrybuty a stan {#a-brief-interlude-props-vs-state}

W Reakcie wyróżniamy dwa modele danych: atrybuty i stan. Bardzo ważne jest zrozumienie, czym dokładnie modele te się różnią. Dla przypomnienia rzuć okiem na [oficjalną dokumentację Reacta](./state-and-lifecycle.html). Możesz także zajrzeć do odpowiedzi [FAQ: Jaka jest różnica między stanem a atrybutami?](./faq-state.html#what-is-the-difference-between-state-and-props)
