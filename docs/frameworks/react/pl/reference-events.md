...
Poniższa dokumentacja dotyczy `SyntheticEvent` (pol. *zdarzenie syntetyczne*) - klasy opakowującej zdarzenia, będącej częścią systemu obsługi zdarzeń Reacta. Samej [obsłudze zdarzeń w Reakcie](./handling-events.html) poświęciliśmy osobny rozdział.
...
> Uwaga:
>
> Od wersji 17, wywołanie metody `e.persist()` nie robi nic, ponieważ `SyntheticEvent` nie jest już współdzielony w [puli](./legacy-event-pooling.html).
...
Więcej informacji odnośnie zdarzenia `onChange` opisaliśmy w rozdziale pt. ["Formularze"](./forms.html).
