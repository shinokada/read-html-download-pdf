...
Ta strona zawiera szegółowe odniesienie do definicji klasy reactowego komponentu. Zakłada ona, że znasz fundamentalne zagadnienia Reacta, takie jak [komponenty i właściwości](./components-and-props.html), i [stan i cykl życia](./state-and-lifecycle.html). Jeśli nie, zapoznaj się najpierw z nimi.
...
**Stanowczo odradadzamy tworzenie własnych klas bazowych komponentów.** W Reactowych komponentach [wielokrotne użycie kodu jest osiągane przede wszystkim przez kompozycję, a nie dziedziczenie](./composition-vs-inheritance.html).

>Uwaga:
>
>React nie zmusza cię do stosowania składni klasy ze standardu ES6. Jeśli wolisz jej uniknąć, możesz zamiast niej użyć modułu `create-react-class` lub podobnej niestandardowej abstrakcji. Aby dowiedzieć się więcej, zobacz rozdział [React bez ES6](./react-without-es6.html).
...
- **Reactowe elementy.** Zwykle tworzone poprzez [JSX](./introducing-jsx.html). Na przykład, `<div />` i `<MyComponent />` są reactowymi elementami, które instruują Reacta, aby, odpowiednio, wyrenderował węzeł drzewa DOM, lub inny zdefiniowany przez użytkownika komponent.
- **Tablice i fragmenty.** Pozwalają na zwrócenie wielu elementów z metody render. Po więcej szczegółów odwiedź dokumentację [fragmentów](./fragments.html).
- **Portale**. Pozwalają na wyrenderowanie elementów potomnych w innym poddrzewie DOM. Po więcej szczegółów odwiedź dokumentację [portali](./portals.html).
...
* Inicjalizacji [stanu lokalnego](./state-and-lifecycle.html) przez przypisanie obiektu do `this.state`.
* Związania [metody obsługującej zdarzenia](./handling-events.html) z instancją komponentu.
...
2. `info` - Obiekt z kluczem `componentStack` zawierający [informację o tym, który komponent wyrzucił ten wyjątek](./error-boundaries.html#component-stack-traces).
...
* [Przewodnik po stanie i cyklu życia](./state-and-lifecycle.html)
...
`this.props` zawiera właściwości, które zostały zdefiniowane przez przywołującego tego komponentu. Po wprowadzenie do właściwości, odwiedź [Komponenty i właściwości](./components-and-props.html).
...
Po informacje na temat stanu, odwiedź [Stan i cykl życia](./state-and-lifecycle.html).
