...
Jeśli wywołasz metodę [`ReactDOM.hydrateRoot()`](./react-dom-client.html#hydrateroot) na węźle, który zawiera już znaczniki wyrenderowane po stronie serwera, React zachowa je i dołączy jedynie procedury obsługi zdarzeń. Poprawi to wydajność i wrażenia przy pierwszym ładowaniu strony.
...
Jeżeli planujesz używać Reacta po stronie klienta w celu dodania znacznikom interaktywności, nie używaj tej metody. Zamiast niej użyj [`renderToNodeStream`](#rendertonodestream) na serwerze i [`ReactDOM.hydrateRoot()`](./react-dom-client.html#hydrateroot) po stronie klienta.
...
Jeśli wywołasz [`ReactDOM.hydrateRoot()`](./react-dom-client.html#hydrateroot) na węźle, który zawiera już taki kod wyrenderowany po stronie serwera, React zachowa go i jedynie doda do niego procedury obsługi zdarzeń, zapewniając szybkie wstępne ładowanie stronie.
...
Jeśli planujesz używać Reacta po stronie klienta, aby dodać aplikacji nieco interakcyjności, nie używaj tej metody. Zamiast niej skorzystaj z [`renderToString`](#rendertostring) po stronie serwera, a następnie [`ReactDOM.hydrateRoot()`](./react-dom-client.html#hydrateroot) po stronie klienta.
