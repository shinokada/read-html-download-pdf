...
W powyższym przykładzie użyliśmy komponentu klasowego, jednak podobna zasada obowiązuje w przypadku komponentów funkcyjnych oraz komponentów tworzonych przy pomocy [`React.memo`](./react-api.html#reactmemo) lub [`React.forwardRef`](./react-api.html#reactforwardref).
