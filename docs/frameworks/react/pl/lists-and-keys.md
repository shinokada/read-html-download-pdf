...
W Reakcie przekształcanie tablic w listy [elementów](./rendering-elements.html) przebiega niemalże identyczne.
 
### Wyświetlanie wielu komponentów {#rendering-multiple-components}
 
Możesz zbudować kolekcje elementów i [dodać je do JSX](./introducing-jsx.html#embedding-expressions-in-jsx) używając nawiasów klamrowych `{}`.
...
Zazwyczaj będziesz wyświetlać listy wewnątrz [komponentu](./components-and-props.html).
...
Jeżeli chcesz dowiedzieć się więcej, tutaj znajdziesz [szczegółowe wyjaśnienie, dlaczego klucze są niezbędne](./reconciliation.html#recursing-on-children).
...
Dla przykładu, jeżeli [wyodrębnisz](./components-and-props.html#extracting-components) komponent `ListItem`, ustaw klucz na elementach `<ListItem />` wewnątrz tablicy zamiast na elemencie `<li>` wewnątrz komponentu `ListItem`.
...
JSX pozwala na [wstawienie dowolnego wyrażenia](./introducing-jsx.html#embedding-expressions-in-jsx) wewnątrz nawiasów klamrowych, dzięki temu możemy użyć wyniku funkcji `map()` bez przypisywania go do zmiennej:
...
Czasami dzięki temu kod staje się bardziej przejrzysty, jednak łatwo doprowadzić do nadużywania tego sposobu. Tak samo jak w JavaScripcie, wybór należy do Ciebie, czy warto wyodrębnić zmienną, aby zwiększyć czytelność. Zapamiętaj, że jeżeli ciało funkcji `map()` jest zbyt zagnieżdżone, może to być odpowiedni moment na [wyodrębnienie komponentu](./components-and-props.html#extracting-components).
