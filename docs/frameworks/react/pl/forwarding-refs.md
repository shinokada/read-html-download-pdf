...
Przekazywanie referencji (ang. *ref forwarding*) to technika, w której [referencję](./refs-and-the-dom.html)
...
Inne komponenty używające `FancyButton` **z reguły nie potrzebują** [mieć dostępu do referencji](./refs-and-the-dom.html) do wewnętrznego elementu `button`.
...
1. Tworzymy [referencję reactową](./refs-and-the-dom.html) wywołując `React.createRef` i przypisujemy ją do stałej `ref`.
...
Omawiana technika może okazać się wyjątkowo przydatna w [komponentach wyższego rzędu](./higher-order-components.html) (KWR; ang. *Higher-Order Components* lub *HOC*). Zacznijmy od przykładu KWR-a, który wypisuje w konsoli wszystkie właściwości komponentu:
