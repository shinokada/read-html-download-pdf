...
>**Wskazówka:**
>
>Łatwo pomylić elementy z szerzej znaną koncepcją "komponentów". Komponenty przedstawimy w [kolejnym rozdziale](./components-and-props.html). Elementy są tym, z czego komponenty "są zbudowane". Zachęcamy do przeczytania tego rozdziału przed przejściem dalej.
...
Aby wyrenderować reactowy element w węźle drzewa DOM, najpierw przekaż element DOM do [`ReactDOM.createRoot()`](./react-dom-client.html#createroot), a następnie przekaż element reactowy do `root.render()`:
...
Wywołuje on [`root.render()`](./react-dom.html#render) z wewnątrz funkcji zwrotnej [`setInterval()`](https://developer.mozilla.org/pl/docs/Web/API/Window/setInterval) co sekundę.

>**Wskazówka:**
>
>W praktyce większość reactowych aplikacji wywołuje `root.render()` tylko raz. W kolejnych rozdziałach dowiemy się, jak można taki kod wyizolować do [komponentów stanowych](./state-and-lifecycle.html).
>
...
![inspektor DOM pokazujący cząstkowe aktualizacje](./docs/granular-dom-updates.gif)
