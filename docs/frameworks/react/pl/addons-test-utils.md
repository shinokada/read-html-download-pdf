...
W [`przykładach`](./testing-recipes.html) znajdziesz więcej szczegółów na temat zachowania funkcji `act()`, jak również przykłady jej użycia.
...
Obiekt `Simulate` posiada odpowiednie metody [dla każdego ze zdarzeń obsługiwanego przez Reacta](./events.html#supported-events).
