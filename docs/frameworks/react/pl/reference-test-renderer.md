...
Podobnie jak [funkcja pomocnicza `act()` z `react-dom/test-utils`](./test-utils.html#act), `TestRenderer.act` przygotowuje komponent do późniejszych asercji. Używaj tej wersji funkcji `act()` do opakowania wywołań `TestRenderer.create` i `testRenderer.update`.
