...
在 React 中，將 array 轉變成 [element](./rendering-elements.html) 列表幾乎是一樣的方式。

### Render 多個 Component {#rendering-multiple-components}

你可以建立一系列的 element 並用大括號 `{}` [將它們包含在 JSX 裡面](./introducing-jsx.html#embedding-expressions-in-jsx)。
...
一般來說，你會在一個 [component](./components-and-props.html) 內 render 列表。
...
如果你想了解更多，請參考[深度剖析 key 的必要性](./reconciliation.html#recursing-on-children)一文。

### 用 Key 抽離 Component {#extracting-components-with-keys}

Key 只有在周遭有 array 的情境中才有意義。

例如，如果你要[抽離](./components-and-props.html#extracting-components)一個 `ListItem` component 的話，你應該把 key 放在 array 裡的 `<ListItem />` element 上，而不是把它放在 `ListItem` 裡面的 `<li>` element 上。
...
JSX 讓你在大括號中[嵌入任何表達式](./introducing-jsx.html#embedding-expressions-in-jsx)，所以我們能夠 inline `map()` 的結果：
...
有時候這會產生更乾淨的程式碼，但這種風格也可能被濫用。就像 JavaScript 一樣，是否要將變數抽取出來以增加可讀性完全是看你的決定。請記得，如果 `map()` 的程式碼層級變得過度巢狀，也許就是使用[抽離 component](./components-and-props.html#extracting-components)的時候了。
