...
* ✅ 在自定義的 Hook 中呼叫 Hook。（我們將會[在下一頁](./hooks-custom.html)了解它們）。
...
[Create React App](./create-a-new-react-app.html#create-react-app) 預設包含此 plugin。
...
**你現在可以先跳過，下一頁將解釋如何打造[你自己的 Hook](./hooks-custom.html)。**在這頁，我們將會繼續解釋這些規則背後的原因。

## 解說 {#explanation}

如我們[先前所學到的](./hooks-state.html#tip-using-multiple-state-variables)，我們可以在單一的 component 中使用多個 State 或 Effect Hook：
...
最後, 我們準備好學習[撰寫你自己的 Hook](./hooks-custom.html)！自定義的 Hook 讓你能結合由 React 提供的 Hook 到你自己的抽象化中，而且在不同的 component 間重複使用相同的 stateful 邏輯。
