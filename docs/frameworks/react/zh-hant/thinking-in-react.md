...
如果你需要幫助來執行這一步的話，請參考這份 [React 文件](./getting-started.html)。

### 簡短的插曲：Props 和 State {#a-brief-interlude-props-vs-state}

React 中有兩種「模型」資料： props and state。理解兩者的差別至關重要。若你不確定兩者的差別，請瀏覽 [React 的官方文件](./state-and-lifecycle.html)以及 [FAQ：props 和 state 有何不同？](./faq-state.html#what-is-the-difference-between-state-and-props)。
