...
近似於 [`測試工具` 中的 `act()` helper](./test-utils.html#act)，`TestRenderer.act` 會為了 assertions 準備一個 component。使用這個版本的 `act()` 來統合 `TestRenderer.create` 和 `testRenderer.update` 的使用。
