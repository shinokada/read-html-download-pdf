...
>im [nächsten Abschnitt](./components-and-props.html) behandeln.
...
Um ein Element in den root-DOM-Knoten zu rendern, muss du zuerst das DOM-Element an [`ReactDOM.createRoot()`](./react-dom-client.html#createroot) übergeben und danach an `root.render()`:
...
Jede Sekunden wird [`root.render()`](./react-dom.html#render) mit Hilfe einer Callback-Funktion von [`setInterval()`](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setInterval) aufgerufen.

>**Hinweis:**
>
>In der Praxis rufen die meisten React-Anwendungen `root.render()` nur einmal auf. Im nächsten Abschnitt lernen wir, wie solch ein Code in einzelne [Zustandskomponenten](./state-and-lifecycle.html) gekapselt wird.
>
...
![DOM-Inspektor mit detaillierten Updates](./docs/granular-dom-updates.gif)
