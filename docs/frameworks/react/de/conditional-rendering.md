...
Im unteren Beispiel erstellen wir eine [zustandsbehaftete Komponente](./state-and-lifecycle.html#adding-local-state-to-a-class), die wir `LoginControl` nennen.
...
Es können [beliebige Ausdrücke in JSX](./introducing-jsx.html#embedding-expressions-in-jsx) verwendet werden, indem man sie in geschweifte Klammern schreibt. Das beinhaltet auch den logischen `&&` Operator aus JavaScript. Dieser kann beim bedingten Darstellen eines Elements sehr nützlich sein.
...
Wie in JavaScript auch, ist es dir überlassen welche Methode du für passend hälst und was du und dein Team lesbarer findet. Bedenke: Wenn Bedingungen zu komplex werden, könnte ein guter Zeitpunkt sein um sie in eine eigene [Komponente auszulagern](./components-and-props.html#extracting-components).
