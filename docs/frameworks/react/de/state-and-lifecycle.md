...
Diese Seite stellt das Konzept von States und Lifecycles in einer React-Komponente vor. Hier kannst du eine [detailiertere API Referenz für Komponenten](./react-component.html) finden.

Schauen wir uns das Beispiel der tickenden Uhr aus dem [vorherigen Abschnitt](./rendering-elements.html#updating-the-rendered-element) an. Im Kapitel [Elemente rendern](./rendering-elements.html#rendering-an-element-into-the-dom), haben wir nur einen Weg kennengelernt, das UI zu aktualisieren. Wir rufen `root.render()` auf, um die gerenderte Ausgabe zu ändern:
...
Wir haben schon [vorher erwähnt](./components-and-props.html#functional-and-class-components), dass Komponenten die als Klassen definiert werden, über einige zusätzliche Features verfügen. Der lokale State ist genau das: Ein Feature, welches nur in Klassen verfügbar ist.
