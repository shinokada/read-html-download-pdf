...
Du kannst [React in einer Minute zu einer beliebigen HTML-Seite hinzufügen](./add-react-to-a-website.html). Danach kannst du entweder die React-Anteile deiner Anwendung stetig erweitern, oder es nur für gewisse in sich geschlossene, dynamische Bestandteile benutzen (zum Beispiel für Formulare).

### Erstelle eine neue React-App {#create-a-new-react-app}

Am Besten fängst du ein neues React-Projekt mit [einer einfachen HTML Seite und ein paar Script-Tags an](./add-react-to-a-website.html). Innerhalb einer Minute hast du alles fertig aufgesetzt!
...
* Wenn du vorzugsweise Konzepte Schritt für Schritt lernst, fang mit unserem [Leitfaden zu den Hauptkonzepten](./hello-world.html) an.
...
Wenn du es bevorzugst **Konzepte Schritt für Schritt** zu lernen, dann ist der [Leitfaden zu den Hauptkonzepten](./hello-world.html) genau das Richtige für dich um anzufangen. Alle Kapitel bauen aufeinander auf, somit verpasst du nichts, wenn du alles nacheinander in der richtigen Reihenfolge liest.

### Denke in React {#thinking-in-react}

Viele Benutzer schreiben, dass es nach dem Lesen von [Thinking in React](./thinking-in-react.html), bei ihnen "klick" gemacht hat.
...
Wenn du einmal mit den [Hauptkonzepten](./hello-world.html) vertraut bist und ein wenig mit React herumgespielt hast, bist du vielleicht an den fortgeschrittenen Konzepten interessiert. Dieser Abschnitt zeigt dir mächtige React-Features, wie [context](./context.html) und [refs](./refs-and-the-dom.html), diese finden aber weniger häufig Verwendung.

### API Referenz {#api-reference}

Dieser Abschnitt der Dokumentation ist nützlich, wenn du mehr über die React API erfahren möchtest.
Zum Beispiel die [API Referenz zu `React.Component`](./react-component.html) beschreibt detailiert wie `setState()` funktioniert und für welchen Einsatz die unterschiedlichen Lifecycle-Methoden nützlich sind.

### Glossar and FAQ {#glossary-and-faq}

Das [Glossar](./glossary.html) bietet dir einen Überblick über die Begriffe, die am häufigsten in der React Dokumentation vorkommen.
Es gibt außerdem ein FAQ, in welchem wir kleine Fragen zu allgemeinen Themen wie: [AJAX Requests](./faq-ajax.html), [States in Komponenten](./faq-state.html) oder [Dateistrukturen](./faq-structure.html) bearbeiten.
