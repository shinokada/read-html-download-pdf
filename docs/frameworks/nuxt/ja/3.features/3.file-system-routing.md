...
#### Info
ページ間を遷移するためには [NuxtLink コンポーネント](./features/nuxt-components#the-nuxtlink-component)を使ってください。
...
#### Next
静的なサイトのために[動的なルーティングを生成する](./concepts/static-site-generation)
...
#### Warning
親コンポーネント（`.vue` ファイル）内に [NuxtChild コンポーネント](./features/nuxt-components#the-nuxtchild-component)を含めることを忘れないでください。
...
- `nuxt.config.js` 内で [router.extendRoutes](./configuration-glossary/configuration-router#extendroutes) プロパティを使う
...
#### Next
[Router Base プロパティ](./configuration-glossary/configuration-router#base)
...
#### Warning
[名前付きビュー](./features/file-system-routing#nested-routes)を使うルートを追加する場合、対応する名前付き `components` の `chunkNames` を追加することを忘れないでください。
...
#### Next
[extendRoutes プロパティ](./configuration-glossary/configuration-router#extendroutes)
...
#### Next
[fallback プロパティ](./configuration-glossary/configuration-router#fallback)
...
#### Next
[mode プロパティ](./configuration-glossary/configuration-router#mode)
...
#### Next
[parseQuery / stringifyQuery プロパティ](./configuration-glossary/configuration-router#parsequery--stringifyquery)
...
#### Next
[scrollBehavior プロパティ](./configuration-glossary/configuration-router#scrollbehavior)
...
#### Next
[trailingSlash プロパティ](./configuration-glossary/configuration-router#trailingslash)
