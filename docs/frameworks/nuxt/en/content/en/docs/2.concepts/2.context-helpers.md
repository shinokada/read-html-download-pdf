...
![](../../../../static/img/docs/context.svg)

The `context` object is available in specific Nuxt functions like [asyncData](./features/data-fetching#async-data), [plugins](./directory-structure/plugins), [middleware](./directory-structure/middleware) and [nuxtServerInit](./directory-structure/store#the-nuxtserverinit-action). It provides _additional_ and often optional information about the current request to the application.
...
Learn more about the different context keys in our [Internals Glossary](./internals-glossary/context)

### Using page parameters for your API query

The context directly exposes possible dynamic parameters of the route via `context.params`. In the following example, we call an API via the `nuxt/http` module using a dynamic page parameter as part of the URL. Modules, like the [nuxt/http](https://http.nuxtjs.org/) module, can expose own functions which are then available through the [context.app](./internals-glossary/context#app) object.
...
#### Next
Check out the Internals Glossary book for more examples of the [redirect method](./internals-glossary/context#redirect)
