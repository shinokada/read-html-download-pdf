...
Hooks are [listeners to Nuxt events](./internals-glossary/internals) that are typically used in Nuxt modules, but are also available in `nuxt.config.js`.
menu: hooks
...
Internally, hooks follow a naming pattern using colons (e.g., `build:done`). For ease of configuration, you can structure them as an hierarchical object when using `nuxt.config.js` (as exemplified above) to set your own hooks. See [Nuxt Internals](./internals-glossary/internals) for more detailed information on how they work.

## List of hooks

- [Nuxt hooks](./internals-glossary/internals-nuxt#hooks)
- [Renderer hooks](./internals-glossary/internals-renderer#hooks)
- [ModulesContainer hooks](./internals-glossary/internals-module-container#hooks)
- [Builder hooks](./internals-glossary/internals-builder#hooks)
- [Generator hooks](./internals-glossary/internals-generator#hooks)
...
To begin, you [can change `router.base`](./configuration-glossary/configuration-router#base); Update your `nuxt.config.js`:
