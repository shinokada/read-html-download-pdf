...
All [modules](./directory-structure/modules) will be called within context of `ModuleContainer` instance.
...
Inside [modules](./directory-structure/modules) context we can use this instead:
...
**Note:** You can use `mode` or `.client` and `.server` modifiers with `fileName` option to use plugin only in client or server side. (See [plugins](./directory-structure/plugins#name-conventional-plugin) for all available options)
...
Pushes middleware into [options.serverMiddleware](./configuration-glossary/configuration-servermiddleware).

### extendBuild (fn)

Allows easily extending webpack build config by chaining [options.build.extend](./configuration-glossary/configuration-build#extend) function.

### extendRoutes (fn)

Allows easily extending routes by chaining [options.build.extendRoutes](./configuration-glossary/configuration-router#extendroutes) function.
