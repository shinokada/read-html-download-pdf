...
The `context` provides additional objects/params from Nuxt to Vue components and is available in special nuxt lifecycle areas like [`asyncData`](./features/data-fetching#async-data), [`fetch`](./features/data-fetching), [`plugins`](./directory-structure/plugins), [`middleware`](./directory-structure/middleware#router-middleware) and [`nuxtServerInit`](./directory-structure/store#the-nuxtserverinit-action).
...
Vuex Store instance. **Available only if the [vuex store](./directory-structure/store) is set**.
...
Environment variables set in `nuxt.config.js`, see [env API](./configuration-glossary/configuration-env).
...
#### Info
It's not possible to use `redirect` or `error` in [client-side Nuxt plugin](./directory-structure/plugins#client-or-server-side-only) due to hydration errors (client content would be different from what it'd expect from the server).
...
The actual [runtime config](./configuration-glossary/configuration-runtime-config).
