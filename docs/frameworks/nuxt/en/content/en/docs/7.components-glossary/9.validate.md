...
`validate` is called every time before navigating to a new route. It will be called server-side once (on the first request to the Nuxt app) and client-side when navigating to further routes. This method takes the [`context`](./internals-glossary/context) object as an argument.
...
You can also check some data in your [store](./directory-structure/store) for example (filled by [`nuxtServerInit`](./directory-structure/store#the-nuxtserverinit-action) before action):
