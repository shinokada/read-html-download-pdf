...
If your `fetch()` accepts a `context` argument, it will be treated like a legacy fetch hook. This functionality is deprecated, and should be replaced with either `asyncData` or an [anonymous middleware.](./directory-structure/middleware#anonymous-middleware)
...
#### Info
You can access the Nuxt [context](./concepts/context-helpers) within the fetch hook using `this.$nuxt.context`.
...
#### Warning
`asyncData` is only available for [pages](./directory-structure/pages) and you don't have access to `this` inside the hook.
...
#### Next
Learn more about the [watchQuery property](./components-glossary/watchquery) and see the list of available [keys in context](./concepts/context-helpers).
