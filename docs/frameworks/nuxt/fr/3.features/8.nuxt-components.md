...
Le composant `<Nuxt>` ne peut être utilisé que dans les [layouts](./concepts/views#layouts).
...
`<NuxtLink>` utilise le [préfétching intelligent](./features/nuxt-components#the-nuxtlink-component) par défaut.
