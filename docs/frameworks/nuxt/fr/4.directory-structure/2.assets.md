...
#### Next
Voir les [ressources webpack](./directory-structure/assets#webpack-assets).
...
#### Next
Pour ajouter des polices de caractères externes telles que des Google Fonts, se référer le chapitre sur le [SEO et les méta tags](./features/meta-tags-seo#external-resources).
...
Si on souhaite changer la configuration d'un des loaders, on trouvera plus de détails à la page de [`build.extend`](./configuration-glossary/configuration-build#extend).
