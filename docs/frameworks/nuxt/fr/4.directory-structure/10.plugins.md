...
Pour davantage d'informations sur les options, veuillez vous référer à la [configuration du build](./configuration-glossary/configuration-build#transpile).
...
On pourrait avoir envie de personnaliser les plugins ou changer l'ordre des plugins créé par Nuxt. Cette fonction accepte un tableau d'objets [plugins](./configuration-glossary/configuration-plugins) et retourne la même chose, réarrangé.
