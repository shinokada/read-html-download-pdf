...
Pour récupérer de la data depuis une API de manière asynchrone dans les composants, on peut utiliser le hook [`fetch()`](./features/data-fetching#the-fetch-method) de Nuxt.
...
#### Next
Se référer au chapitre sur [fetch()](./features/data-fetching#the-fetch-method) pour avoir davantage d'informations sur son fonctionnement.
...
#### Next
Voir [la propriété components](./configuration-glossary/configuration-components) pour d'autres méthodes de contrôle du nom des composants.
