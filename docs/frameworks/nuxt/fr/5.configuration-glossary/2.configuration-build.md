...
#### Warning
**Attention**: Les clés `isClient` et `isServer` fournies n'ont rien à voir avec celles présentes dans le [`context`](./internals-glossary/context). Elles ne sont **pas** dépréciées. Il ne faut en outre pas utiliser `process.client` et `process.server` ici car ils seront `undefined` à ce niveau.
...
> Nuxt nous permet d'utiliser nos propres templates qui seront render basés sur les [modules](./directory-structure/modules).
...
Dans cet exemple, `ky` sera transpilé par Babel si Nuxt n'est pas en [mode moderne](./configuration-glossary/configuration-modern).
...
> On peut fournir nos propres fichiers personnalisés à surveiller avant de régénérer après des modifications. Cette fonctionnalité est surtout utile à utiliser avec [modules](./directory-structure/modules).
