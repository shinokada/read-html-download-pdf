...
Cette option est utilisée par `nuxt generate` avec la [cible statique](./features/deployment-targets#static-hosting) pour éviter de re-build lorsque les fichiers traqués n'ont pas été modifiés.
...
- **Déprécié !**
- Il faut utiliser [build.html.minify](./configuration-glossary/configuration-build#htmlminify) à la place.
