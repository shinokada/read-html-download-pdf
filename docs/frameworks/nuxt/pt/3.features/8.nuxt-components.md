...
#### Warning
O componente `<Nuxt>` apenas pode ser usando dentro dos [esquemas](./concepts/views#layouts).
...
O `<NuxtLink>` também vem com a [pré-requisição inteligente](./features/nuxt-components#the-nuxtlink-component) fora da caixa.
