...
#### Warning
**Aviso:** As chaves `isClient` e `isServer` fornecidas são separadas das chaves disponíveis dentro do [`contexto`](./internals-glossary/context). Elas **não** estão depreciadas. Não use `process.client` e `process.server` aqui neste ponto quando eles forem `undefined`.
...
> O Nuxt permite você fornecer os seus próprios modelos os quais serão renderizados com base na configuração do Nuxt. Esta funcionalidade é especialmente útil para uso com [módulos](./directory-structure/modules).
...
Neste exemplo, o `ky` será transpilado pelo Babel se o Nuxt não estiver no [modo moderno](./configuration-glossary/configuration-modern).
...
> Você pode fornecer os seus ficheiros personalizados para observar e regerar depois de mudanças. Esta funcionalidade é especialmente útil para ser usada com [módulos](./directory-structure/modules).
