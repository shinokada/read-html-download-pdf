...
> もし 404 のケースをハンドリングしていない場合、[`handleError`](../30-advanced/20-hooks.html#shared-hooks-handleerror) によって表示が行われます。
...
`src/routes/archive/[page]` のようなルート(route)は `/archive/3` にマッチしますが、`/archive/potato` にもマッチしてしまいます。これを防ぎたい場合、パラメータ文字列(`"3"` や `"potato"`)を引数に取ってそれが有効なら `true` を返す _matcher_ を [`params`](../50-api-reference/10-configuration.html#files) ディレクトリに追加することで、ルート(route)のパラメータを適切に定義することができます…
