...
> これらのファイルの場所は [`config.kit.files.hooks`](../50-api-reference/10-configuration.html#files) で設定できます。
...
この関数は SvelteKit のサーバーが [リクエスト](../10-getting-started/40-web-standards.html#fetch-apis-request) を受けるたびに (アプリの実行中であろうと、[プリレンダリング](../20-core-concepts/40-page-options.html#prerender)であろうと) 実行され、[レスポンス](../10-getting-started/40-web-standards.html#fetch-apis-response) を決定します。リクエストを表す `event` オブジェクトと、ルート(route)をレンダリングしレスポンスを生成する `resolve` という関数を受け取ります。これにより、レスポンスのヘッダーやボディを変更したり、SvelteKitを完全にバイパスすることができます (例えば、プログラムでルート(routes)を実装する場合など)。
...
[`sequence` ヘルパー関数](../50-api-reference/30-modules.html#sveltejs-kit-hooks)を使用すると、複数の `handle` 関数呼び出しを追加することができます。
...
この関数は _想定される_ エラー (`@sveltejs/kit` からインポートされる [`error`](../50-api-reference/30-modules.html#sveltejs-kit-error) 関数でスローされるエラー) の場合は呼び出されません。
