...
> [プロジェクトの設定](../50-api-reference/10-configuration.html) を編集することで、`src/routes` から別のディレクトリに変更することができます。
...
`+page.svelte` コンポーネントはアプリのページを定義します。デフォルトでは、ページは最初のリクエストではサーバー ([SSR](../60-appendix/30-glossary.html#ssr)) でレンダリングされ、その後のナビゲーションではブラウザ ([CSR](../60-appendix/30-glossary.html#csr-and-spa)) でレンダリングされます。
...
この関数は `+page.svelte` とともに実行されます。サーバーサイドレンダリング中はサーバーで実行され、クライアントサイドナビゲーション中はブラウザで実行されます。API の詳細は [`load`](../20-core-concepts/20-load.html) をご参照ください。
...
これらに関するより詳しい情報は [page options](../20-core-concepts/40-page-options.html) をご覧ください。

### +page.server.js

`load` 関数をサーバー上でのみ実行できるようにしたい場合 — 例えば、データベースからデータを取得したり、API キーのようなプライベートな[環境変数](../50-api-reference/30-modules.html#$env-static-private)にアクセスしたりする必要がある場合 — `+page.js` を `+page.server.js` にリネームし、`PageLoad` 型を `PageServerLoad` に変更します。
...
クライアントサイドナビゲーション中は、SvelteKit はサーバーからこのデータを読み込みます。つまり、その戻り値は [devalue](https://github.com/rich-harris/devalue) によってシリアライズできなければならないということです。この API の詳細については [`load`](../20-core-concepts/20-load.html) をご参照ください。

`+page.js` のように、`+page.server.js` は [page options](../20-core-concepts/40-page-options.html) (`prerender`、`ssr`、`csr`) をエクスポートできます。

また、`+page.server.js` ファイルは _actions_ をエクスポートできます。`load` がサーバーからデータを読み取る場合、`actions` は `<form>` 要素を使用してサーバーにデータを書き込むことができます。これらの使い方を学ぶには、[form actions](../20-core-concepts/30-form-actions.html) セクションをご参照ください。
...
> エラーが [`handle`](../30-advanced/20-hooks.html#server-hooks-handle) の内側や [+server.js](#server) リクエストハンドラ の内側で発生した場合は、`+error.svelte` は使用されません。
...
デフォルトでは、各レイアウトはその上にあるレイアウトを継承します。そうしたくない場合は、[advanced layouts](../30-advanced/10-advanced-routing.html-routing#advanced-layouts) が役に立つでしょう。

### +layout.js

`+page.svelte` が `+page.js` からデータを読み込むように、`+layout.svelte` コンポーネントは `+layout.js` の [`load`](../20-core-concepts/20-load.html) 関数からデータを取得することができます。
...
`+layout.js` が [page options](../20-core-concepts/40-page-options.html) (`prerender`、`ssr`、`csr`) をエクスポートする場合、それは子ページのデフォルトとしても使用されます。
...
> しばしば、ページ間をナビゲーションしているときにレイアウトデータが変更されないことがあります。SvelteKit は必要に応じてインテリジェントに [`load`](../20-core-concepts/20-load.html) 関数を再実行します。

### +layout.server.js

サーバー上でレイアウトの `load` 関数を実行するためには、それを `+layout.server.js` に移動し、`LayoutLoad` 型を `LayoutServerLoad` に変更します。

`+layout.js` と同様に、`+layout.server.js` では [page options](../20-core-concepts/40-page-options.html) — `prerender`、`ssr`、`csr` をエクスポートすることができます。
...
> 一般的には、ブラウザからサーバーにデータを送信する方法としては [form actions](../20-core-concepts/30-form-actions.html) のほうがより良い方法です。
...
コンポーネントやモジュールが複数のルート(routes)から必要な場合、[`$lib`](../50-api-reference/30-modules.html#$lib) にそれらを配置すると良いでしょう。
