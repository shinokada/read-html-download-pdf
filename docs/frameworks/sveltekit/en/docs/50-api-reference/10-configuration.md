...
Run when executing `vite build` and determines how the output is converted for different platforms. See [Adapters](../20-core-concepts/50-adapters.html).
...
- `publicPrefix` — a prefix that signals that an environment variable is safe to expose to client-side code. See [`$env/static/public`](../50-api-reference/30-modules.html#$env-static-public) and [`$env/dynamic/public`](../50-api-reference/30-modules.html#$env-dynamic-public). Note that Vite's [`envPrefix`](https://vitejs.dev/config/shared-options.html#envprefix) must be set separately if you are using Vite's environment variable handling - though use of that feature should generally be unnecessary.
...
- `hooks` — the location of your client and server hooks (see [Hooks](../30-advanced/20-hooks.html))
- `lib` — your app's internal library, accessible throughout the codebase as `$lib`
- `params` — a directory containing [parameter matchers](../30-advanced/10-advanced-routing.html-routing#matching)
- `routes` — the files that define the structure of your app (see [Routing](../20-core-concepts/10-routing.html))
- `serviceWorker` — the location of your service worker's entry point (see [Service workers](../30-advanced/40-service-workers.html))
...
Options related to [creating a package](../30-advanced/70-packaging.html).
...
- `base` — a root-relative path that must start, but not end with `/` (e.g. `/base-path`), unless it is the empty string. This specifies where your app is served from and allows the app to live on a non-root path. Note that you need to prepend all your root-relative links with the base value or they will point to the root of your domain, not your `base` (this is how the browser works). You can use [`base` from `$app/paths`](../50-api-reference/30-modules.html#$app-paths-base) for that: `<a href="{base}/your-page">Link</a>`. If you find yourself writing this often, it may make sense to extract this into a reusable component.

## prerender

See [Prerendering](../20-core-concepts/40-page-options.html#prerender). An object containing zero or more of the following:
...
If you set `pollInterval` to a non-zero value, SvelteKit will poll for new versions in the background and set the value of the [`updated`](../50-api-reference/30-modules.html#$app-stores-updated) store to `true` when it detects one.
