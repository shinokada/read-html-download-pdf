...
Before a [`+page.svelte`](../20-core-concepts/10-routing.html#page-page-svelte) component (and its containing [`+layout.svelte`](../20-core-concepts/10-routing.html#layout-layout-svelte) components) can be rendered, we often need to get some data. This is done by defining `load` functions.
...
Notice that the type changed from `PageLoad` to `PageServerLoad`, because server-only `load` functions can access additional arguments. To understand when to use `+page.js` and when to use `+page.server.js`, see [Shared vs server](../20-core-concepts/20-load.html#shared-vs-server).
...
> In some environments this is derived from request headers during server-side rendering. If you're using [adapter-node](../20-core-concepts/50-adapters.html#supported-environments-node-js), for example, you may need to configure the adapter in order for the URL to be correct.
...
A server-only `load` function can get and set [`cookies`](../50-api-reference/40-types.html#sveltejs-kit-cookies).
...
If an error is thrown during `load`, the nearest [`+error.svelte`](../20-core-concepts/10-routing.html#error) will be rendered. For _expected_ errors, use the `error` helper from `@sveltejs/kit` to specify the HTTP status code and an optional message:
...
If an _unexpected_ error is thrown, SvelteKit will invoke [`handleError`](../30-advanced/20-hooks.html#shared-hooks-handleerror) and treat it as a 500 Internal Error.
...
You can also re-run `load` functions that apply to the current page using [`invalidate(url)`](../50-api-reference/30-modules.html#$app-navigation-invalidate), which re-runs all `load` functions that depend on `url`, and [`invalidateAll()`](../50-api-reference/30-modules.html#$app-navigation-invalidateall), which re-runs every `load` function.
...
- It declared a dependency on a specific URL via [`fetch`](#making-fetch-requests) or [`depends`](../50-api-reference/40-types.html#sveltejs-kit-loadevent), and that URL was marked invalid with [`invalidate(url)`](../50-api-reference/30-modules.html#$app-navigation-invalidate)
- All active `load` functions were forcibly re-run with [`invalidateAll()`](../50-api-reference/30-modules.html#$app-navigation-invalidateall)

Note that re-running a `load` function will update the `data` prop inside the corresponding `+layout.svelte` or `+page.svelte`; it does _not_ cause the component to be recreated. As a result, internal state is preserved. If this isn't what you want, you can reset whatever you need to reset inside an [`afterNavigate`](../50-api-reference/30-modules.html#$app-navigation-afternavigate) callback, and/or wrap your component in a [`{#key ...}`](https://svelte.dev/docs#template-syntax-key) block.
