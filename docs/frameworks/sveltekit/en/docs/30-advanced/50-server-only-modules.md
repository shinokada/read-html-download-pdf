...
The `$env/static/private` and `$env/dynamic/private` modules, which are covered in the [modules](../50-api-reference/30-modules.html) section, can only be imported into modules that only run on the server, such as [`hooks.server.js`](../30-advanced/20-hooks.html#server-hooks) or [`+page.server.js`](../20-core-concepts/10-routing.html#page-page-server-js).
