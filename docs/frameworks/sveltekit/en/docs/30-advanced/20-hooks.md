...
> You can configure the location of these files with [`config.kit.files.hooks`](../50-api-reference/10-configuration.html#files).
...
This function runs every time the SvelteKit server receives a [request](../10-getting-started/40-web-standards.html#fetch-apis-request) — whether that happens while the app is running, or during [prerendering](../20-core-concepts/40-page-options.html#prerender) — and determines the [response](../10-getting-started/40-web-standards.html#fetch-apis-response). It receives an `event` object representing the request and a function called `resolve`, which renders the route and generates a `Response`. This allows you to modify response headers or bodies, or bypass SvelteKit entirely (for implementing routes programmatically, for example).
...
You can add call multiple `handle` functions with [the `sequence` helper function](../50-api-reference/30-modules.html#sveltejs-kit-hooks).
...
This function is not called for _expected_ errors (those thrown with the [`error`](../50-api-reference/30-modules.html#sveltejs-kit-error) function imported from `@sveltejs/kit`).
