...
`<style>` tag in Markdown is always [scoped](https://vue-loader.vuejs.org/guide/scoped-css.html). To have global style overrides, check out the [customization section](custom/directory-structure.html)
...
For local assets, put them into the [`public` folder](custom/directory-structure.html.html) and reference them with **leading slash**.
...
Learn more about [frontmatter configurations](custom/.html)
