...
`onMount` does not run inside a [server-side component](04-run-time.html).
...
Creates an event dispatcher that can be used to dispatch [component events](03-template-syntax.html). Event dispatchers are functions that can take two arguments: `name` and `detail`.
...
The `svelte/store` module exports functions for creating [readable](04-run-time.html), [writable](04-run-time.html) and [derived](04-run-time.html) stores.

Keep in mind that you don't *have* to use these functions to enjoy the [reactive `$store` syntax](02-component-format.html-script-4-prefix-stores-with-$-to-access-their-values) in your components. Any object that correctly implements `.subscribe`, unsubscribe, and (optionally) `.set` is a valid store, and will work both with the special syntax, and with Svelte's built-in [`derived` stores](04-run-time.html).

This makes it possible to wrap almost any other reactive state handling library for use in Svelte. Read more about the [store contract](02-component-format.html-script-4-prefix-stores-with-$-to-access-their-values-store-contract) to see what a correct implementation looks like.
...
* `easing` (`function`, default `t => t`) — an [easing function](04-run-time.html)
...
As with [`tweened`](04-run-time.html) stores, `set` and `update` return a Promise that resolves if the spring settles.
...
The `svelte/transition` module exports seven functions: `fade`, `blur`, `fly`, `slide`, `scale`, `draw` and `crossfade`. They are for use with Svelte [`transitions`](03-template-syntax.html).
...
* `easing` (`function`, default `linear`) — an [easing function](04-run-time.html)
...
* `delay` (`number`, default 0) — milliseconds before starting
* `duration` (`number`, default 400) — milliseconds the transition lasts
* `easing` (`function`, default `cubicInOut`) — an [easing function](04-run-time.html)
...
* `delay` (`number`, default 0) — milliseconds before starting
* `duration` (`number`, default 400) — milliseconds the transition lasts
* `easing` (`function`, default `cubicOut`) — an [easing function](04-run-time.html)
...
The `crossfade` function creates a pair of [transitions](03-template-syntax.html) called `send` and `receive`. When an element is 'sent', it looks for a corresponding element being 'received', and generates a transition that transforms the element to its counterpart's position and fades it out. When an element is 'received', the reverse happens. If there is no counterpart, the `fallback` transition is used.
...
* `duration` (`number` | `function`, default 800) — milliseconds the transition lasts
* `easing` (`function`, default `cubicOut`) — an [easing function](04-run-time.html)
* `fallback` (`function`) — A fallback [transition](03-template-syntax.html) to use for send when there is no matching element being received, and for receive when there is no element being sent. 
...
The `hydrate` option instructs Svelte to upgrade existing DOM (usually from server-side rendering) rather than creating new elements. It will only work if the component was compiled with the [`hydratable: true` option](05-compile-time.html). Hydration of `<head>` elements only works properly if the server-side rendering code was also compiled with `hydratable: true`, which adds a marker to each element in the `<head>` so that the component knows which elements it's responsible for removing during hydration.
...
Svelte components can also be compiled to custom elements (aka web components) using the `customElement: true` compiler option. You should specify a tag name for the component using the `<svelte:options>` [element](03-template-syntax.html).
...
By default, custom elements are compiled with `accessors: true`, which means that any [props](03-template-syntax.html) are exposed as properties of the DOM element (as well as being readable/writable as attributes, where possible).
...
You can import a Svelte component directly into Node using [`svelte/register`](04-run-time.html).
